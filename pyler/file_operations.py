...
class FileOperation():
    ...
    def note_label_box(self, label_text, yol):
        ...
        if self.notebook.get_n_pages() >= 1:
            GLib.idle_add(self.notebook.set_show_tabs, True)
    ...
    def open(self, dosya):
        """ open given filepath """
        if dosya.lower().endswith(".json"):
#            self.wikieditor.current_editor.set_highlight_current_line(False)
#            self.wikieditor.current_buffer.set_highlight_syntax(False)
#            self.wikieditor.current_editor.set_wrap_mode(gtk.WrapMode.NONE)
            ...
            def on_change(_, response):
                ...
                try:
                    with open(bilgi ,"w", encoding="utf-8") as dosya:
                        ...
                        GLib.idle_add(self.notebook.remove_page, page)
                except IOError as msj:
                    mesaj(f"{bilgi}\nFile cannot saved..\
