...
class MenuItems():
    ...
    def __init__( self, wiki_editor):
        ...
        "wiki_editor" : [
            ...
            ("Open Directory", self.open_directory, "<primary><super>O"),
        ...
        "more_wiki": [
            ( "Run Terminal", self.run_terminal,  "<super>T", True),
...
#################################################################
    ...
    def show_preview(self, widget, *_):
        ...
        if widget.get_state() == GLIB_FALSE:
            GLib.idle_add(self.wiki_editor.preview.set_visible, True)
            widget.set_state(GLIB_TRUE)
            return True

        GLib.idle_add(self.wiki_editor.preview.set_visible, False)
        widget.set_state(GLIB_FALSE)
        self.wiki_editor.redkit.try_close()
        return False

    def run_terminal(self, widget, *_):
        """
        show/hide webkit preview
        """
        parent = self.wiki_editor.get_child().get_last_child().get_end_child().get_start_child()

        if widget.get_state() == GLIB_FALSE:
            widget.set_state(GLIB_TRUE)

            terminal = Terminal()
            terminal.to_dir(self.gl_b["directory"])
            terminal.add_action(self.wiki_editor)

            parent.set_end_child(terminal)
            self.wiki_editor.add_custom_styling(terminal)

            GLib.idle_add(terminal.set_visible, True)
            return True

        if terminal:=parent.get_end_child():
            terminal.emit("eof")
    ...
    def show_hide_toolbar(self, action, *_):
        ...
        """
        if action.get_state() == GLIB_FALSE:
            GLib.idle_add(self.toolbar.scroll.set_visible, True)
            action.set_state(GLIB_TRUE)

        else:
            GLib.idle_add(self.toolbar.scroll.set_visible, False)
            action.set_state(GLIB_FALSE)
...
#####  set up text using sed regex
    ...
    def open_directory(self, *_):
        """
        builds and open choosen directory
        """
        dialog = get_filechooser(self.wiki_editor,
                    desc_="Open a directory..")
        dialog.select_folder(self.wiki_editor, None, OpenDirectory, self.wiki_editor)
